...
$(document).ready(function () {
    ...
    if (href.includes("mucdich")) {
        let mucdichindex = String(href).lastIndexOf("=") + 1;
        let mucdich = href.substring(mucdichindex);

        var selectedRadio = document.getElementById("mucdich-"+mucdich);
        selectedRadio.checked = true;

        $("#mucdich-"+mucdich).trigger("click");
    }
});
...
function LoadList(list) {
    ...
    for (let cource of list) {
        ...
        if (cource.Disscount != 0) {
            cource_card += `<div class="disscount-card">
                                <p>-${cource.Disscount}%</p>
                            </div>`;
        }
        var diss_price = cource.Price * ((100 - cource.Disscount) / 100);
        cource_card += `<a href="detail.html?id=${cource.Id}">
                                    ...
                                    <div class="item-footer list-group-horizontal">
                                        ...
                                        <p class="price"><b>${ToVietNamDong(diss_price)}</b></p>
                                    </div>
                                </a>
                            </div>`;
